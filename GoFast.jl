...
# ╔═╡ 94e380ac-b4c6-11ec-269c-c780239c09c5
begin
	using JSON
	using PlutoUI
	using Random
...
$(@bind selected_file PlutoUI.FilePicker([MIME("application/json")]))
"""
...
if jsondict["actions"]["1"] != "Process._id0->Process._id0 { 1, fast!, updateFast(), counterFast++ }"
	md"""
# Warning: Unexpected strategy format!

Strategy actions do not match expected values. Did you change the UPPAAL model? 
	"""
end

# ╔═╡ edc4906e-4b8c-407a-a99b-6792200d0b98
if jsondict["pointvars"][1] != "x" || jsondict["pointvars"][2] != "t"
	md"""
# Warning: Unexpected strategy format!

The field `pointvars` should contain x and t as the only two elements, in that order.
	"""
end

# ╔═╡ f4681c7f-6d06-491d-bab7-3acc4e82ed8b
function get_predicted_outcome(vars, tree)
	# Base case
	...
	# Recursion
	var_index = tree["var"] + 1 # Julia indexes start at 1
	var = vars[var_index]
	bound = tree["bound"]
	if var >= bound
		get_predicted_outcome(vars, tree["high"])
	else
		get_predicted_outcome(vars, tree["low"])
	end
end
...
function get_action(slow_tree, fast_tree, x, t)
	predicted_slow = get_predicted_outcome([x, t], slow_tree)
	predicted_fast = get_predicted_outcome([x, t], fast_tree)
	if predicted_slow <= predicted_fast
		:slow
	else
		:fast
	end
